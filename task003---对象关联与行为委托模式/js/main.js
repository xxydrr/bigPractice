(function(_) {
    ...
    initDetail.on('edit', function() {
        var editInit = Object.create(Edit);
        editInit.init(this.getData());
        editInit.on('confirm', function() {
            var todos = [];
            var index = '';
    ...
    function renderTaskList(arr) {
        ...
        for(var i = 0;i < arr.length;i++) {
            ...
            if(oldTask !== null) {
                ...
                todo.init(arr[i]);
                todo.container.index = '' + arr[i].index;
                // console.log(todo.index);

                oldTask.widget.taskWrap.appendChild(todo.container);
    ...
    function renderCGList(arr) {
        ...
        for(var i = 0; i < arr.length; i++) {
            if(arr[i].name === '默认分类') {
                obj = Object.create(Category);
                obj.init({name: arr[i].name, number: arr[i].number, canDelete: false});
                obj.category.id = 'default';
            }
            else{
                obj = Object.create(Category);
                obj.init({name: arr[i].name, number: arr[i].number});
            }
            obj.category.index = '' + i;
            var subArr =  arr[i].subCategory
            if(subArr.length > 0) {
                for(var j = 0;j < subArr.length; j++) {
                    subObj = Object.create(SubCategory);
                   subObj.init({name: subArr[j].name, number: subArr[j].number});
    ...
    $.delegate(cgList, '.delete-i', 'click', function() {
        ...
        var selectIndex = '';
        var ModalDelete = Object.create(Modal);
        ModalDelete.init({hasFlag: true,text:'是否确定删除？'});
        ModalDelete.on('confirm', function() {
            if(index.indexOf('-') < 0) {
        ...
        ModalDelete.appendTo(document.body);
     
            
    });
    // 给新增分类的按钮添加点击事件
    $.click(addCG, function() {
        var ModalAdd = Object.create(Modal);
        ModalAdd.init({hasFlag: false});
        ModalAdd.on('confirm', function() {
        ...
        ModalAdd.appendTo(document.body);
    });
    // 2-5.给新增任务的按钮注册点击事件
